...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			DECLARE @countryCode VARCHAR(8), @distance FLOAT;
			...
			BEGIN
				SET @inventory = (SELECT [Inventory].[quantity]
								 ...
								 WHERE [Inventory].[idProduct] = @productId);
			END;
			...
			IF(@inventory IS NOT NULL AND @inventory > @quantity)
				BEGIN
					...
					-- Add sale
					INSERT INTO Sale(idUser, idSeller, idDelievery, idProduct, total, quantity) 
					VALUES (@idUser,@idSeller,@lastDelivery,@productId,@total, @quantity);

				END;
			-- Product does not exist
			ELSE
				print 'Invertory error'
		END;
...
--Example
EXEC purchase  
@productId = 2 ,@quantity  = 2 , @idDelieveryPerson = 2
@idUser = 4, @idSeller = 1, @locationName = 'house',
@locationX = 53.34680724107667, @locationY =-6.262823939323426;

SELECT * FROM Sale
SELECT * FROM Delievery
SELECT * FROM ProductsDB.DBO.Product where idProduct=2